...
function drawBlockDepthPlot(ctx, drawpoints){
	...
	for (i=0; i<drawpoints.length; i++){
		...
		if (((drawpoints[i][0])>=viewport.start) && (drawpoints[i][0]<=viewport.end)){

			if (drawpoints[i][1]>maxdepth) {
				maxdepth=drawpoints[i][1];
			}
		}
		else if (drawpoints[i][0]<viewport.start){
	...
	ctx.moveTo(viewport.canvasstart, ypos);
	ctx.lineTo(viewport.canvasstart, ypos-((startval/maxdepth)*plotheight));

	for (i=0; i<drawpoints.length; i++){

		if (((drawpoints[i][0])>=viewport.start) && (drawpoints[i][0]<=viewport.end)){

			ctx.lineTo((viewport.xscalingfactor*(drawpoints[i][0]-viewport.start))+viewport.canvasstart, ypos-((drawpoints[i][1]/maxdepth)*plotheight));
	...
	if (endval!=0){
		ctx.lineTo(viewport.canvasend, ypos-((endval/maxdepth)*plotheight));
	}
	ctx.lineTo(viewport.canvasend, ypos);
	ctx.closePath();
	ctx.stroke();
	ctx.fill();
	ctx.beginPath();
	ctx.moveTo(viewport.canvasstart, ypos);
	ctx.lineTo(viewport.canvasstart, ypos-plotheight);
	ctx.lineTo(viewport.canvasstart-2, ypos-plotheight);
	ctx.stroke();
	ctx.save();
	ctx.translate( viewport.canvasstart+2, ypos-plotheight);
	ctx.textAlign = "left";
